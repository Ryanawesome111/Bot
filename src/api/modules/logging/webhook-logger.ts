...
export class WebhookLogger {
  ...
  async feedback(message: string) {
    const webhook = await this.get(process.env.FEEDBACK_CHANNEL_ID, 'Mr.Helper - Feedback');
    if (!webhook) return;
  ...
  async vote(userId: string, votes: number) {
    const webhook = await this.get(process.env.VOTE_CHANNEL_ID, 'Mr.Helper - Vote');
    if (!webhook) return;
